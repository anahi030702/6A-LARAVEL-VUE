...
export default {
  ...
  data() {
    return {
      form: {
        ...
        screening_id: this.sale.screening_id,
        seats : this.sale.seats,
  ...
  methods: {
    evactualizar() {
      router.put('/sales/update', this.form);
    },
    loadCinemas() {
      axios.get('/cinemas/prueba')
        .then(response => {
          this.cinemas = response.data.cinemas;
          this.loadRooms(this.form.cinema_id);
          this.loadScreenings(this.form.room_id);
        })
        .catch(error => {
...
<template>
    <MoviesLayout>
      ...
      <form class="row needs-validation" novalidate @submit.prevent="evactualizar">
        <div class="row mb-2">
